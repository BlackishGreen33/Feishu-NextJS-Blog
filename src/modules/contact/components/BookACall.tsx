...
const BookACall = () => {
  ...
  if (!SITE_BOOK_CALL_URL) {
    return <EmptyState message={messages.contact.noScheduleLink} />;
  }

  return (
    <div className='space-y-5 pb-2'>
      <h3 className='text-lg font-medium'>{messages.contact.scheduleTitle}</h3>
      <Link
        href={SITE_BOOK_CALL_URL}
        target='_blank'
      ...
      >
        ...
        <div className='flex items-center gap-5 text-sm dark:text-neutral-200'>
          <div className='flex items-center gap-2'>
            <FiClock size={18} />
            <span>{site.bookCallDuration}</span>
          </div>
          <div className='flex items-center gap-2'>
            <FiVideo size={18} />
            <span>{site.bookCallTool}</span>
          </div>
        </div>
      </Link>
    </div>
